...
class WrMemTest(clockFreq : Int = 100000000, uartBaudrate : Int = 115200, txFile : String) extends Module {
  val io = IO(new Bundle {
    val instr = Output(UInt(16.W))
  })

  val programmer = Module(new Sender(clockFreq, uartBaudrate, txFile))
  val wrInstrMem = Module(new WrInstrMem(8, clockFreq, uartBaudrate))
